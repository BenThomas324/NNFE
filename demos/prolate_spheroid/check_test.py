...
Y = nnfe.sampler.Y
nx, ny = 25, 25
Ps = onp.linspace(Y[:, 0].min(), Y[:, 0].max(), nx)
TCas = onp.linspace(Y[:, 1].min(), Y[:, 1].max(), ny)
Y_grid = onp.stack(onp.meshgrid(Ps, TCas), axis=-1)
